import { Injectable } from '@angular/core';
import { HttpClient, HttpHeaders } from '@angular/common/http';
import { environment } from '../../../environments/environment.prod';
import { Observable } from 'rxjs';

@Injectable({
  providedIn: 'root'
})
export class DepartmentService {

     //#region Declare variables
     title:any;
     //#endregion
     
     //#region  constructor
     constructor(private http:HttpClient) { }
     //#endregion
   
       //#region Options
       httpOptionsWithTocken = { headers: new HttpHeaders({ 'Authorization': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..55LorE6Fclj3buy1Qw8wZ6CEe_ifW5jxwHe25wHRWsQ', 'Accept': ' */*' }) };
       //#endregion
   
       Get(): Observable<any> {
         return this.http.get<any>(`${environment.Server_URL}/Department`);
       }
       
       GetDepartment(id:any): Observable<any> {
         return this.http.get<any>(`${environment.Server_URL}/Department?id=${id}`);
       }
    
       GetAllDepartment(): Observable<any> {
         return this.http.get<any>(`${environment.Server_URL}/Department`);
       }
     
       Insert(Data:any): Observable<any> {
         return this.http.post<any>(`${environment.Server_URL}/Department`,Data);
       }
   
       Update(id:any,Data:any): Observable<any> {
         return this.http.post<any>(`${environment.Server_URL}/Department/${id}`,Data);
       }
   
       Delete(ClientId:any): Observable<any> {
         return this.http.get<any>(`${environment.Server_URL}/Department/delete/${ClientId}`);
       }
}
